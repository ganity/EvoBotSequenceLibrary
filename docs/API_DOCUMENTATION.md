...
为了解决机器人在使用动作序列时可能遇到的中文编码问题，系统为每个动作序列自动生成了英文名称字段（`english_name`）。

#### 功能特点
- **自动生成**：系统根据中文名称自动生成对应的英文标识符
- **语义关联**：英文名称与中文名称保持语义关联，便于理解和使用
- **唯一标识**：每个英文名称在系统中唯一，可作为动作的标准标识符
- **编码安全**：避免中文字符在不同系统间传输时出现乱码问题

#### 生成规则
1. **词汇映射**：使用内置的中英文词汇映射表进行翻译
2. **分类前缀**：自动添加分类前缀增强语义性（如：`arm_movement_`）
3. **格式规范**：转换为小写，使用下划线分隔，符合标识符规范
4. **唯一性保证**：系统确保生成的英文名称在数据库中唯一

#### 使用场景
- **机器人交互**：机器人可通过英文名称调用动作，避免编码问题
- **API调用**：开发者可使用英文名称作为稳定的标识符
- **文件命名**：下载的动作文件使用英文名称命名，便于管理
- **跨平台兼容**：确保在不同操作系统和编码环境下的兼容性

#### 示例映射
| 中文名称 | 英文名称 | 说明 |
|---------|---------|------|
| 左臂挥手 | `arm_movement_left_arm_wave` | 手臂动作分类下的左臂挥手动作 |
| 右臂拥抱 | `arm_movement_right_arm_hug` | 手臂动作分类下的右臂拥抱动作 |
| 点头确认 | `head_nod_confirm` | 头部动作的点头确认 |
| 微笑打招呼 | `smile_greeting` | 表情动作的微笑打招呼 |
...
#### 自动功能
- **英文名称生成**：系统会根据中文名称自动生成英文标识符（`english_name`）
- **文件处理**：自动解析EBS文件获取动作元数据（时长、帧数、采样率等）
- **哈希计算**：自动计算文件MD5哈希值确保完整性
...
- **文件名**: 下载的文件使用英文名称命名（如：`arm_movement_left_arm_wave.ebs`），避免中文编码问题
- **失败**: 返回JSON错误响应
...
### 5. 英文名称使用
- 优先使用英文名称作为动作标识符，确保跨平台兼容性
- 在机器人程序中使用英文名称调用动作，避免编码问题
- 文件存储和管理使用英文名称，便于维护

```python
def get_sequence_by_english_name(english_name):
    """通过英文名称获取动作序列"""
    url = f"http://your-server:9189/api/v1/sequences/list"
    params = {"limit": 1}
    
    result = api_request_with_retry(url, params=params)
    if result and result['sequences']:
        for seq in result['sequences']:
            if seq.get('english_name') == english_name:
                return seq
    return None

def robot_execute_action(english_name, robot_id):
    """机器人执行指定英文名称的动作"""
    # 1. 通过英文名称查找动作
    sequence = get_sequence_by_english_name(english_name)
    if not sequence:
        logging.error(f"未找到动作: {english_name}")
        return False
    
    # 2. 下载动作文件（使用英文名称作为文件名）
    download_url = f"http://your-server:9189/api/v1/sequences/{sequence['id']}/download"
    params = {
        'robot_id': robot_id,
        'compensation': True,
        'safety_check': True
    ...
    response = requests.get(download_url, params=params)
    if response.status_code == 200:
        filename = f"{english_name}.ebs"
        with open(filename, 'wb') as f:
            f.write(response.content)
        
        logging.info(f"成功下载动作文件: {filename}")
        return True
...
### Q: 英文名称是如何生成的？
A: 系统使用内置的中英文词汇映射表自动生成英文名称：
1. **词汇映射**：将中文词汇转换为对应的英文词汇
2. **分类前缀**：自动添加分类前缀（如：`arm_movement_`）
3. **格式规范**：转换为小写，使用下划线分隔
4. **唯一性**：确保在系统中唯一，可作为稳定的标识符

### Q: 为什么要使用英文名称？
A: 英文名称解决了以下问题：
- **编码兼容**：避免中文字符在不同系统间传输时出现乱码
- **机器人交互**：机器人可通过英文名称调用动作，确保稳定性
- **文件命名**：下载的文件使用英文名称，便于跨平台管理
- **API调用**：开发者可使用英文名称作为稳定的标识符

### Q: 可以自定义英文名称吗？
A: 目前系统自动生成英文名称以确保一致性和唯一性。如需特殊需求，可联系管理员进行配置。
